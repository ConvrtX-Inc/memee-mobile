...
import { View, Text, FlatList, TouchableOpacity, StyleSheet, TextInput, ScrollView, Dimensions, Modal, Image, ImageBackground, } from 'react-native';
...
export default function OrganizeBadges(props) {
    ...
    function filterSelectFN(index) {
        ...
        var seachedCity = earndBadgesToStare.filter(function search(badge) { return badge.rarity.toUpperCase().includes(rarity.toUpperCase()) })
        if (index == 0) {
            setEarndeBadges(earndBadgesToStare)
        }
        else {
    ...
    function getProgressBadge() {
        fetch(global.address + "GetBadgesProgress" + "/" + global.userData.user_id, {
            method: 'get',
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json',
            ...
            .then((responseJson) => {
                ...
                responseJson.BadgesProgress.forEach(function (element) {
                    ...
                    element.percentProgress = ((doneProgress * 100) / element.total_items) / 100;
                });
    ...
    function searchBadgesFN(value) {
        ...
        var seachedCity = earndeBadges.filter(function search(badge) { return badge.badge_name.toUpperCase().includes(value.toUpperCase()) })
        if (value == "") {
            setEarndeBadges(earndBadgesToStare)
        }
        else {
        ...
        <View style={{ flex: 1, paddingHorizontal: "5%", paddingTop: "5%", backgroundColor: global.colorPrimary}}>
            <ScrollView>
                ...
                <View style={{ flexDirection: 'row', }}>
                    ...
                    <Text style={[styles.title, { fontFamily: global.fontSelect, color: global.colorIcon }]}>Organize Badges</Text>
                </View>


                <View style={{ flexDirection: 'row', width: '100%' }}>
                    <View style={{ flexDirection: 'row', height: 55, width: "75%", backgroundColor: '#201E23', alignSelf: 'flex-start', borderRadius: 32 }}>
                        <TouchableOpacity>
                            <Image
                        ...
                        <TextInput style={{ marginLeft: 5, color: "#fff", width: '78%', fontFamily: global.fontSelect }}
                    ...
                    {showFilter == false ?
                        <TouchableOpacity onPress={() => showFilterFN(true)}>
                            <View style={{ justifyContent: 'center', alignItems: 'center', backgroundColor: '#201E23', height: 55, width: 55, borderRadius: 30, marginLeft: 10 }}>
                                <Image
                ...
                {showFilter == false ? null
                    :
                    <View>
                        <Text style={{ color: global.colorTextPrimary, marginTop: 10, marginLeft: '5%' }}>Filter</Text>
                        {/* Filter FlatList */}
                        <FlatList
                            horizontal={true}
                            ...
                            renderItem={({ item, index }) =>
                                <TouchableOpacity onPress={() => filterSelectFN(index)}>
                                    ...
                                    <LinearGradient
                                        ...
                                        style={{ marginLeft: 10, justifyContent: 'center', alignSelf: 'center', borderRadius: 22, }}
                                    >

                                        <View style={{ paddingHorizontal: 16, borderWidth: 1, borderColor: '#676767', borderRadius: 22, paddingVertical: 10, }}>
                                            <Text style={{ color: item.txtClr, fontFamily: global.fontSelect }}>#{item.title}</Text>
                                        </View>
                ...
                <View style={{ flexDirection: 'row', width: '90%', alignSelf: 'center', marginTop: 22, justifyContent: 'space-between', alignItems: 'center' }}>
                    <Text style={{ color: global.colorTextPrimary, fontSize: 16, fontWeight: 'bold' }}>Earned Badges</Text>
                    {earnBadgeCollaps == true ?
                        <TouchableOpacity style={{ height: 11, width: 19 }} onPress={() => showEarnedBadgesFN(false)}>
                            <Image
                        ...
                        :
                        <TouchableOpacity style={{ height: 11, width: 19 }} onPress={() => showEarnedBadgesFN(true)}>
                            <View>
                                <Image
                                    style={{ height: 8, width: 15, tintColor: global.colorIcon }}
                                    resizeMode='stretch'
                ...
                <LinearGradient
                    ...
                    style={{ alignItems: 'center', marginTop: (windowWidth * 5) / 100, borderRadius: (windowWidth * 10) / 100, width: (windowWidth * 90) / 100, paddingVertical: (windowWidth * 7) / 100, backgroundColor: global.colorSecondary }}>
                        <FlatList
                            // horizontal={true}
                            numColumns={4}
                            data={earndeBadges}
                            renderItem={({ item, index }) =>
                                <TouchableOpacity onPress={() => selectBadgesToShow(index)}>
                                    <View style={{ width: (windowWidth * 20) / 100, alignItems: 'center', marginVertical: (windowWidth * 1) / 100, }} >
                                        ...
                                        <Text style={{ color: global.colorTextPrimary, opacity: 0.5, fontSize: 11, marginTop: (windowWidth * 0.5) / 100 }}>{item.badge_name}</Text>
                                    </View>
                ...
                <View style={{ flexDirection: 'row', width: '90%', marginTop: 22, alignSelf: 'center', justifyContent: 'space-between', alignItems: 'center' }}>
                    <Text style={{ color: global.colorTextPrimary, fontSize: 16, fontWeight: 'bold' }}>Locked Badges</Text>
                    {lockedBadgeCollaps == true ?
                        <TouchableOpacity style={{ height: 11, width: 19 }} onPress={() => showLockedBadgesFN(false)}>
                            <Image
                                style={{ height: 8, width: 15,  tintColor: global.colorIcon }}
                                resizeMode='stretch'
                                source={require('../../images/upVector.png')}
                        ...
                        :
                        <TouchableOpacity style={{ height: 11, width: 19 }} onPress={() => showLockedBadgesFN(true)}>
                            <View>
                                <Image
                                    style={{ height: 8, width: 15, tintColor: global.colorIcon }}
                                    resizeMode='stretch'
                ...
                {lockedBadgeCollaps == true ?
                    <FlatList
                        ...
                        data={lockedBadges}
                        renderItem={({ item, index }) =>
                            <View style={{ padding: (windowWidth * 3.5) / 100, borderRadius: (windowWidth * 5) / 100, backgroundColor: '#292929', width: (windowWidth * 40) / 100, marginHorizontal: (windowWidth * 1.5) / 100, marginTop: (windowWidth * 4) / 100 }} >

                                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                                    <View style={{ borderRadius: (windowWidth * 3) / 100, height: (windowWidth * 10.5) / 100, width: (windowWidth * 10.5) / 100, backgroundColor: '#FFFFFF22', justifyContent: 'center', alignItems: 'center' }}>
                                        <Image
                                            style={{ width: (windowWidth * 6) / 100, height: (windowWidth * 6) / 100 }}
                                            resizeMode='stretch'
                                            source={
                                                { uri: item.image_url }
                                            }
                                    ...
                                    <TouchableOpacity onPress={() => setModalVisible(!modalVisible)}>
                                        <Image
                                            style={{ width: (windowWidth * 4) / 100, height: (windowWidth * 5.3) / 100 }}
                                            resizeMode='stretch'
                                ...
                                <Text style={{ color: "#fff", fontSize: 13, marginTop: (windowWidth * 0.8) / 100 }}>{item.badge_name}</Text>
                                <Text style={{ color: "#8E8DAF", fontSize: 10, marginTop: (windowWidth * 0.8) / 100 }}>{item.description}</Text>
                                <Text style={{ color: "#8E8DAF", fontSize: 10, marginTop: (windowWidth * 1.4) / 100 }}>Rarity: {item.rarity}</Text>
                                <Text style={{ color: "#8E8DAF", fontSize: 10, marginTop: (windowWidth * 4.5) / 100, marginBottom: (windowWidth * 1.5) / 100 }}>{item.remaining_items} points need</Text>

                                <ProgressBar style={{ marginBottom: 10 }} progress={item.percentProgress} color={'#FFCD2F'} />
            ...
            >
                <View style={styles.centeredView}>
                    <View style={styles.modalView}>
                        ...
                        <Image
                            style={{ width: (windowWidth * 35) / 100, height: (windowWidth * 25) / 100, marginBottom: 20 }}
                            resizeMode='stretch'
                            source={require("../../images/1stPrize.png")}
                        />
                        <Text style={[styles.modalText, { fontFamily: global.fontSelect }]}>1st Place Badge</Text>

                        <Text style={{ color: "#fff", opacity: 0.5, fontSize: 13, textAlign: 'center', fontFamily: global.fontSelect }}>
                        ...
                        >
                            ...
                            >
                                <Text style={[styles.textStyle, { fontFamily: global.fontSelect }]}>Done</Text>
                            </LinearGradient>
                        </TouchableOpacity>
                    </View>
                </View>
