...
import { View, Text, TouchableOpacity, MaskedViewIOS, Dimensions, Modal, StyleSheet, ScrollView, Image, FlatList, ImageBackground, ViewBase } from 'react-native';
...
export default function FontScreen() {
    ...
    async function getprofileBgFN() {
        ...
        await fetch(global.address + 'GetStoreItems/' + global.userData.user_id + "/font", {
            method: 'get',
            headers: {
            ...
            .then(async (responseJson) => {
                ...
                for (let i = 0; i < responseJson.StoreItems.length; i++) {
                    responseJson.StoreItems[i].font = asignImageToProductsFN(responseJson.StoreItems[i].item_code, responseJson.StoreItems[i].type)[0].font
                    responseJson.StoreItems[i].name = asignImageToProductsFN(responseJson.StoreItems[i].item_code, responseJson.StoreItems[i].type)[0].themeName
                }
            ...
            await fetch(global.address + 'PurchaseStoreItem', {
                ...
                .then(async (responseJson) => {
                    ...
                    if (responseJson.Status == "201") {
                        ...
                        var sms = "You have purchased "+ iconList[index].name +" successfully."
            ...
            <ScrollView >
                <FlatList
                    ...
                    renderItem={({ item, index }) =>
                        <View style={{ width: (windowWidth * 90) / 100, height: (windowWidth * 43) / 100, backgroundColor: '#201F38', borderRadius: (windowWidth * 10) / 100, alignSelf: 'center', marginTop: 30 }}>
                            ...
                            <TouchableOpacity style={{ marginTop: 26 }} >
                                ...
                                <View style={{ borderColor: '#4A4A4A', borderWidth: 1, borderRadius: 40, width: '93%', height: 58, alignSelf: 'center', justifyContent: 'center', alignItems: 'center' }}>
            ...
            >
                <View style={styles.centeredView}>
                    <View style={styles.modalView}>
                        ...
                        <TouchableOpacity onPress={() => setModalVisible(!modalVisible)} style={{ marginLeft: 'auto', marginTop: -(windowWidth * 11.5) / 100 }}>
                            <Image
                                style={{ height: (windowWidth * 6) / 100, width: (windowWidth * 6) / 100, alignSelf: 'center', }}
                                resizeMode='stretch'
                        ...
                        <Image
                            style={{ height: (windowWidth * 37) / 100, width: (windowWidth * 37) / 100, tintColor: "#73FF66ee", alignSelf: 'center', marginTop: (windowWidth * 5) / 100 }}
                            resizeMode='stretch'
                            source={require("../../../images/success.png")}
                        />

                        <Text style={[styles.modalText, { fontFamily: global.fontSelect, fontSize: 20, marginTop: (windowWidth * 6) / 100 }]}>Successful</Text>
                        <Text style={[styles.modalText, { fontFamily: global.fontSelect, fontSize: 12, opacity: 0.5 }]}> {message}</Text>
