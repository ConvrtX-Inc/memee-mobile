...
export default function ProfileOverlayScreen() {
    ...
    async function getProfileOverlayFN() {
        ...
        await fetch(global.address + 'GetStoreItems/' + global.userData.user_id + "/background_overlay", {
            method: 'get',
            headers: {
            ...
            .then(async (responseJson) => {
                ...
                for (let i = 0; i < responseJson.StoreItems.length; i++) {
                    ...
                    responseJson.StoreItems[i].img = asignImageToProductsFN(responseJson.StoreItems[i].item_code, responseJson.StoreItems[i].type)
    ...
    async function purchaseOverlayFN(index) {
            ...
            <ScrollView >
                ...
                <FlatList
                    ...
                    renderItem={({ item, index }) =>
                        <View style={{ width: (windowWidth * 44) / 100, height: (windowWidth * 67) / 100, backgroundColor: '#201F38', borderRadius: (windowWidth * 10) / 100, alignSelf: 'center', marginVertical: (windowWidth * 5) / 100, marginHorizontal: (windowWidth * 3) / 100 }}>
                            ...
                            <View style={{ alignSelf: 'center', marginTop: -(windowWidth * 4) / 100 }}>
                                <ButtonCoinsShort
                                    title={item.coins}
                                    showAdd="2"
                                    font=""
                                    loading={item.loadIndicat}
                                    onPress={() => purchaseOverlayFN(index)}
                                />
                            </View>
                        </View>
                    }
