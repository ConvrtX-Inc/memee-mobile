...
import { View, Text, FlatList, Switch, Dimensions, BackHandler, Platform, ActivityIndicator, TouchableOpacity, StyleSheet, ScrollView, Modal, Image, ImageBackground, } from 'react-native';
...
export default function AddCoins(props) {
    ...
    function showPurchaseHistory() {
        ...
        fetch(global.address + 'GetTransactionsHistory/' + global.userData.user_id, {
            method: 'get',
            headers: {
    ...
    async function purchaseProducts(index) {
        ...
        RNIap.initConnection().then(() => {
            RNIap.flushFailedPurchasesCachedAsPendingAndroid().catch(() => {
                ...
                purchaseUpdateSubscription = purchaseUpdatedListener((purchase = InAppPurchase) => {
                    ...
                    if (receipt) {
                        ...
                        fetch(global.address + 'PurchaseCoins', {
                            ...
                            .then(async (responseJson) => {
                                console.log('here2')
                                global.userData.coins = parseInt(global.userData.coins) + parseInt(currentProduct.description)
                                ...
                                setProducts(([...forLoader]))
                                console.log('here3')
                                navigation.navigate('CoinsConfirmation', { coins: currentProduct.description });
    ...
    return (
        <View style={{ flex: 1, paddingTop: "5%", backgroundColor: global.colorPrimary }}>
            <ScrollView>
                ...
                <View style={{ flexDirection: 'row', marginLeft: "4%", marginTop: 10 }}>
                    ...
                    <Text style={[styles.title, { fontFamily: global.fontSelect, color: global.colorIcon }]}>Add Coins</Text>
                ...
                <Carousel
                    ...
                    renderItem={({ item, index }) => (
                        <View style={{
                            ...
                            >
                                <Text style={{ fontFamily: global.fontSelect, fontSize: 40, fontWeight: 'bold', textAlign: 'center', color: '#FFB800', marginTop: windowWidth - ((windowWidth * 47) / 100) }}>{item.description}</Text>
                                <Text style={{ fontFamily: global.fontSelect, fontSize: 34, textAlign: 'center', color: '#FFB800', marginTop: - (windowWidth - ((windowWidth * 95) / 100)) }}>Coins</Text>
                                ...
                                >
                                    <LinearGradient
                                        ...
                                        style={{ paddingHorizontal: 35, minWidth: 185, paddingVertical: 15, justifyContent: 'center', alignSelf: 'center', borderRadius: 25, }}
                                    >
                                        {item.loader == true ?
                                            <ActivityIndicator animating={true} color="#ffffff" />
                                            :
                                            <Text style={{ color: '#fff', fontFamily: global.fontSelect }}>Buy in {item.localizedPrice}</Text>
                                        }
                                    </LinearGradient>
                                </TouchableOpacity>
                            </ImageBackground>
                ...
                {transactionHistory.length > 0 ?
                    <View style={{ width: '90%', flexDirection: 'row', marginTop: 20, marginLeft: 15, marginBottom: 10 }}>
                        <Text style={{ color: global.colorTextPrimary , fontFamily: global.fontSelect }}>Purchasing History</Text>
                    </View>
                    :
                    null
                }
                ...
                <FlatList
                    ...
                    renderItem={({ item, index }) => (
                        <View>
                            {/* <TouchableOpacity> */}
                            <View style={{ width: '95%', alignSelf: 'center', marginTop: 5, marginBottom: 5, height: 50, borderRadius: 25, flexDirection: 'row', backgroundColor: '#292929', }} >
                                ...
                                <View style={{ width: "25%", height: 50, borderRadius: 22 }}>
                                    ...
                                    >
                                        <Text style={{ color: '#fff', fontSize: 15, fontFamily: global.fontSelect }}>{Moment(item.datetime).format('DD MMM')}</Text>
                                ...
                                <View style={{ width: '41%', flexDirection: 'row' }}>
                                    ...
                                    <Text style={{ marginLeft: '1%', color: '#FFB800', fontSize: 17, fontWeight: 'bold', marginTop: 11, textAlign: 'center', fontFamily: global.fontSelect }}>{item.coins}</Text>
                                </View>

                                <View style={{ marginLeft: '12%', width: '20%', marginTop: 5 }}>
                                    <LinearGradient
                                        colors={["#74FF67", "#1D8B14"]}
                                        style={{ height: 40, width: '100%', borderRadius: 22, justifyContent: 'center', alignItems: 'center' }}
                                    >
                                        <Text style={{ color: '#fff', fontFamily: global.fontSelect }}>{item.currency + ' ' + item.price}</Text>
                                    </LinearGradient>
                                </View>
                            </View>
                            {/* </TouchableOpacity> */}
