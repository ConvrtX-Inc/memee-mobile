...
import { View, TouchableOpacity, Dimensions, Platform, StyleSheet, Linking, Text, Image} from 'react-native';
import { colors } from '../../Utility/colors'
import { useNavigation } from '@react-navigation/native';
import { useSelector, useDispatch } from 'react-redux';
import Icon from 'react-native-vector-icons/AntDesign'
import { GiftedChat, InputToolbar, Bubble, Send, Avatar, Time} from 'react-native-gifted-chat'
...
const ChatScreen = ({route}) => {
    ...
    function setMessagesView(data){
        ...
        data.messages.forEach(value => {
            messages.push({
                ...
                location: value.Type == 'location' ? {latitude: parseFloat(value.Content.split(',')[0]), longitude: parseFloat(value.Content.split(',')[1])} : "",
    ...
    function getConversation(offset){
        axios.get(`${global.address}getConversation/${conversationId}/${offset}`)
    ...
    function sendMessage(msg, type){
        ...
        axios.post(`${global.address}sendMessage`, data)
            .then(function (response) {
            })
            .catch(function (error) {
                console.log(error);
    ...
    useEffect(() => {
        messaging().onMessage(async remoteMessage => {
            ...
            var msg = {
                ...
                text: remoteMessage.notification.body.trim() == 'Shared this image' ? "" : remoteMessage.notification.body,
                createdAt: remoteMessage.sentTime,
                user: users.find(element => element.userId == remoteMessage.data.objectId),
                image: remoteMessage.notification.body.trim() == 'Shared this image' ? remoteMessage.data.image : null
            }
    ...
    function renderInputToolbar(props) {
        ...
        return (
            <View style={{ marginTop: 40, width: '90%', alignSelf: 'center', marginBottom: 20, }}>
                <InputToolbar
                    {...props}
                    containerStyle={{
        ...
        launchImageLibrary(options, (response) => {
            ...
            if (response.didCancel) {
                ...
                RNS3.put(file, getBucketOptions("chat")).then(response => {
                    ...
                    valueToPush["user"] = users.find(element => element.userId == global.userData.user_id)
            ...
            {otherChatUser ? 
                <View style={styles.header}>
                    ...
                    {otherChatUser.avatar != "" ?
                        <Image source={{uri: otherChatUser.avatar}}
                    ...
                    {otherChatUser.onlineStatus == '1' ?
                        <View>
                            <Image source={require("../../images/online.png")}
                                style={{ height: 18, width: 18, borderRadius: 10, marginLeft: -15, marginTop: 20 }}
                                resizeMode="cover"
                            />
                        </View> 
                    : null }
                    <View style={{flexDirection: 'column'}}>
                        ...
                        {otherChatUser.onlineStatus == '0' && otherChatUser.lastSeen ? <Text style={styles.simpleText}>{getLastSeenFormat(otherChatUser.lastSeen)}</Text> : null }
                    </View>
                </View>
            : null }
            <GiftedChat
...
const styles = StyleSheet.create({
    ...
    sendImage: {
        height: 22, width: 22, tintColor: colors.textColor
    },
    myStyle: {
        alignItems: 'center',
        borderColor: '#fff',
    ...
    addFriendImage: { marginStart: 10, height: 50, width: 50, borderRadius: 50 / 2 },
