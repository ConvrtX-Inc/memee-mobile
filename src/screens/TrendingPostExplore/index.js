...
import { View, Text, RefreshControl, TextInput, TouchableOpacity, Dimensions, ActivityIndicator, MaskedViewIOS, StyleSheet, ScrollView, Image, FlatList, ImageBackground, ViewBase } from 'react-native';
...
export default class TrendingPostExplore extends React.Component {
    ...
    render() {
        ...
        return (
            <View style={{ flex: 1, backgroundColor:  global.colorPrimary, marginBottom: 15, }}>

                <Text style={{ color:  global.colorTextPrimary, marginLeft: '4%', marginTop: 15, fontFamily: global.fontSelect }}>Trending Tags</Text>
                        ...
                        <TouchableOpacity onPress={() => this.tagChangeFN(index)}>
                            ...
                            >
                                ...
                                <View style={{ paddingHorizontal: 16, borderWidth: 1, borderColor: '#676767', borderRadius: 22, paddingVertical: 10, }}>
                                    <Text style={{ color: item.txtClr, fontFamily: global.fontSelect }}>#{item.tag_id}</Text>
                                </View>
                ...
                {this.state.activityIndicateHandle == true ?
                    ...
                    <View style={{height: (windowHeight * 60) / 100, justifyContent:'center', alignItems: 'center'}}>
                        <ActivityIndicator size="large" color={global.colorTextActive} style={{ alignSelf: 'center' }} />
                    </View>
                    :
                    <View flexDirection='row' style={{width: '100%'}} >
                        <FlatList
                        data={this.state.PostData.slice(0,Math.ceil(this.state.PostData.length / 2))}
                        renderItem={({ item, index }) =>
                            <View style={{ borderWidth: 0, borderRadius: 30, }} >

                                <TouchableOpacity onPress={() => this.navigateToExpDetailFN(index)}>
                                 
                                    <ImageBackground source={{ uri: item.PostImage, }} resizeMode='stretch'
                                        style={{
                                            width: '95%', height: 160 + (Math.floor(Math.random() * 3) * 40), margin: 10
                                        }}
                                        resizeMode="cover"
                                        ...
                                        <View style={{ flexDirection: 'row', width: '70%', height: 40, position: 'absolute', bottom: 20 }}>
                                            <View style={{ borderColor: '#fff', borderWidth: 1, borderRadius: 20, height: 36, marginLeft: 20 }}>
                                                <Avatar
                                                    rounded
                                                    size="small"
                                                    source={
                                            ...
                                            <Text style={{ color: '#fff', fontSize: 11, marginLeft: 5, alignSelf: 'center', fontFamily: global.fontSelect }}>{item.UserName}</Text>
                                        </View>
                    ...
                    />
                    <FlatList 
                        data={this.state.PostData.slice((Math.ceil(this.state.PostData.length / 2)), this.state.PostData.length)}
                        renderItem={({ item, index }) => 
                            <View style={{  borderWidth: 0, borderRadius: 30, }} >

                                <TouchableOpacity onPress={() => this.navigateToExpDetailFN(Math.ceil(this.state.PostData.length / 2) + index)}>
                                    <ImageBackground source={{ uri: item.PostImage, }} resizeMode='stretch'
                                        style={{
                                            width: '95%', height: 200 + (Math.floor(Math.random() * 3) * 20), margin: 10
                                        }}
                                        resizeMode="cover"
                                        imageStyle={{ borderRadius: 25 }}>
                                        <LinearGradient
