...
import { View, Text, TouchableOpacity, StyleSheet, Image, FlatList, ImageBackground, ViewBase } from 'react-native';
...
export default function TopScreen({ navigation }) {
    ...
    return (
        <View style={{ flex: 1, backgroundColor: global.colorPrimary, paddingHorizontal: 5 }}>
            <FlatList
                data={searchData}
                    ...
                    <TouchableOpacity onPress={() => navigateToProfile(item)}>
                        <View style={{ marginTop: 15, paddingBottom: 10, width: "100%", height: 70, flexDirection: 'row', alignItems: 'center', borderBottomColor: '#1B1B1B', borderBottomWidth: 1 }}>
                            ...
                            <View style={{ width: '65%', height: 60, marginLeft: 10 }}>
                                <Text style={{ color: global.colorTextPrimary, fontSize: 16, fontWeight: 'bold', marginTop: 4, fontFamily: global.fontSelect }}> {item.name}</Text>
                                <Text style={{ color: global.colorTextPrimary, fontSize: 13, marginTop: 2, fontFamily: global.fontSelect }}>{item.email}</Text>
                            </View>
                ...
                renderItem={({ item, index }) =>
                    <TouchableOpacity onPress={() => getPostsAgainstTag(index)}>
                        <View style={{ marginTop: 15, paddingBottom: 10, width: "100%", height: 70, flexDirection: 'row', alignItems: 'center', borderBottomColor: '#1B1B1B', borderBottomWidth: 1 }}>
                            ...
                            <View style={{ borderWidth: 1, borderColor: '#fff', width: 50, height: 50, backgroundColor: '#0D0219', borderRadius: 28, alignItems: 'center', justifyContent: 'center' }}>
                                <Text style={{ color: '#fff', fontSize: 33, fontFamily: global.fontSelect }}>#</Text>
                            </View>

                            <View style={{ width: '65%', height: 60, marginLeft: 10 }}>
                                <Text style={{ color: global.colorTextPrimary, fontSize: 16, fontWeight: 'bold', marginTop: 4, fontFamily: global.fontSelect }}> {item.tag_id}</Text>
                                <Text style={{ color: global.colorTextPrimary, fontSize: 13, marginTop: 2, fontFamily: global.fontSelect }}>{item.TotalPosts} posts</Text>
                            </View>
            ...
            <View>
                <Text style={{ color: global.colorTextPrimary, fontSize: 16, fontWeight: 'bold', marginTop: 20, fontFamily: global.fontSelect }}> Recent Searches</Text>
                <FlatList
                    data={recentSearch}
                    renderItem={({ item, index }) =>
                        ...
                        <TouchableOpacity onPress={() => navigateToProfile(item)}>
                            <View style={{ marginTop: 15, paddingBottom: 10, width: "100%", height: 70, flexDirection: 'row', alignItems: 'center', borderBottomColor: '#1B1B1B', borderBottomWidth: 1 }}>
                                ...
                                <TouchableOpacity onPress={() => removeSearchItem(index)} style={{ width: '15%', height: 60, marginLeft: 'auto', flexDirection: 'row' }}>
                                    <Image
                                        style={{ height: 18, width: 18, tintColor: global.colorIcon, alignSelf: 'center', marginLeft: '10%' }}
                                        resizeMode='stretch'
                                        source={require('../../images/cross1.png')}
                                    />
                                </TouchableOpacity>
